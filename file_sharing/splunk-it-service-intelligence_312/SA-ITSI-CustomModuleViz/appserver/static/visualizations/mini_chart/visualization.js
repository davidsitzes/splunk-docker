...
/***/ (function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_ARRAY__ = [
	    ...
	    var getChartElementId = function (chartId) {
	        return 'entity-context-chart-' + chartId;
	    };

	    return SplunkVisualizationBase.extend({
	        /**
	         * Initializes visualization base
	         */
	        initialize: function() {
	        ...
	        /**
	         * Called only once per visualization and overrides SplunkVisualizationBase's setupView.
	         * Creates a MiniChart only after the div for the MiniChart is rendered. This is done to avoid the error we get
	         * when rendering a chart that does not have a parent element.
	         */
	        setupView: function () {
	            this.chartOptions = {};
	            this.dfd = $.Deferred();
	            ...
	            var CHART_PARAMS = ['id', 'chartType', 'title', 'subtitleField'];
	            _.each(CHART_PARAMS, function (param) {
	                this.chartOptions[param] = this._getEscapedProperty(param, config);
	            }, this);

	            this.chartOptions.additionalChartOptions = this.getAdditionalChartOptions(config);
	    
	            var that = this;
	            var minichartDivObserver = new MutationObserver(function(mutation) {
	                var minichartDiv = $('#' + header_id + '_minicharts');
	                if (minichartDiv.length) {
	                    minichartDiv.append('<div id="' + getChartElementId(that._getEscapedProperty('id', config)) + '" class="minichart-viz"></div>');
	                    that.chartOptions.el = $('#' + getChartElementId(that.chartOptions.id));
	                    that.miniChart = new MiniChart(that.chartOptions);
	                    this.disconnect();
	                    that.dfd.resolve();
	                }
	            });

	            minichartDivObserver.observe(document.body, {
	                attributes: false, childList: true, characterData: false, subtree: true
	            });
	        },

	        /**
	        * Called everytime the search returns data. Chains updateChart method to the deferred object.
	        *
	        * @param {Object} data: The data object returned by the search.
	        */
	        updateView: function (data) {        
	            this.dfd.done(_.bind(this.updateChart, this, data));
	        },

	        /**
	        * Calls updateView of MiniChart with formatted results.
	        *
	        * @param {Object} data: The data object returned by the search.
	        */
	        updateChart: function (data) {
	            var currentMinichart = this.miniChart;
	            var subtitleIndex = _.findIndex(data.fields, function(element) {return element.name === currentMinichart.options.subtitleField});

	            if (!_.isEmpty(currentMinichart.options.subtitleField) && subtitleIndex > -1) {
	                $('#' + currentMinichart.vizId + '-subtitle').text(data.rows[0][subtitleIndex]);
	            } else {
	                $('#' + currentMinichart.vizId + '-subtitle').text('N/A');
	            }

	            if (data.rows.length > 0)  {
	                var formattedResults = this.formatResults(data);
	                currentMinichart.updateView(formattedResults);
	            }
	        },

	        // Taken from SplunkVisualizationBase
	        ...
	        /**
	        * Returns a specific property from the config
	        *
	        * @param {Object} name: name of the property
	        * @param {Object} config: the config object of the chart
	        */
	        ...
	        /**
	        * Extracts and returns options specific to a particular chart mentioned in the xml view.
	        * These names of these options are prefixed with 'charting_'
	        * for example:
	        *   <option name='SA-ITSI-CustomModuleViz.mini_chart.charting_seriesColors'>["0x5379AF"]</option>
	        * from the above line of config, 'seriesColors' option will be added to additionalChartOptions
	        *
	        * @param {Object} config: the config object of the chart
	        */
	        getAdditionalChartOptions: function (config) {
	            var additionalChartOptions = {};
	            _.each(config, function(value, key) {
	                var chartingIndex = key.indexOf('charting');
	                if (chartingIndex !== -1) {
	                    additionalChartOptions[key.substring(chartingIndex).replace(/_/g, '.')] = value;
	                }
	            });
	            return additionalChartOptions;
	        },

	        /**
	        * Takes the data returned by the search and returns a series object that contains only the part that the minichart needs
	        * in the format that Highcharts uses.
	        *
	        * @param {Object} data: The data object returned by the search.
	        */
	        formatResults: function(data) {
	            var originalCategories = [];
	            dataIndex = _.findIndex(data.fields, function(element) {return element.name === '_time'});
	            // dataIndex is index of _time; the next index will have the actual data to render;
	            var series;
	            var that = this;

	            if (dataIndex == -1) {
	                series = _.map(data.rows, function(row){
	                    return that.formatCellType(row[dataIndex + 1]);
	                });
	            } else {
	                series = _.map(data.rows, function(row) {
	                    return [new Date(row[dataIndex]).getTime(), that.formatCellType(row[dataIndex + 1])];
	                }); 
	            }
	        ...
	        /**
	        * Converts the data value into a number.
	        * Note: Number(null) would give 0, but null should not be converted to 0, because 0 is still a data point on the chart,
	        * whereas we want null to be a discontinuation in the chart.
	        *
	        * @param {string} value: A data point that needs to be rendered.
	        */
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_RESULT__ = function (require, module) {
	    ...
	    var HighchartsMore = __webpack_require__(8);
	    HighchartsMore(Highcharts);
	    var HighchartsGauge = __webpack_require__(9);
	     ...
	     */
	    var MiniChart = Backbone.View.extend({
	        moduleId: module.id,
	         ...
	         * Initialize the view
	         *
	         * @param {Object} options: options of the minichart that come from the options in the xml view
	         * @public
	         */
	        initialize: function (options) {
	            Backbone.View.prototype.initialize.apply(this, arguments);
	            this.options = options;
	            this.vizId = ['mini-chart', this.options.chartType, this.id].join('-');

	            if (!_.contains(this.validChartTypes, this.options.chartType)) {
	                this.$el.html('Invalid chart type');
	                return null;
	            }

	            this.$el.html(this._template({
	                chart_title: this.options.title,
	                viz_id: this.vizId
	            }));

	            // Override default options with the additional options
	            var finalOptions = _.extend({},
	                this.options.additionalChartOptions,
	                {
	                    'id': this.vizId,
	                    'el': $('#' + this.vizId)
	                });
	        ...
	        /**
	         * Renders the chart if it is not already rendered.
	         * Only updates the chart if it already present, instead of rendering again.
	         *
	         * @param {Object} series: series is the formatted result data that is to be given to highcharts.
	         */
	        updateView: function (series) {
	            if (this.chart) {
	                this.chart.series[0].remove();
	                this.chart.addSeries(series[0], true);
	                return;
	            } else {
	                this.render(series);
	            }
	        ...
	        /**
	         * Renders the chart in appropriate div.
	         *
	         * @param {Object} series: data series to be rendered by the chart.
	         */
	        render: function(series) {
	            this.chartOptions = this.getDefaultChartOptions(series);

	            _.each(this.options.additionalChartOptions, function(value, key) {
	                // if the value is supposed to be an array, convert it from string to array
	                // eg convert '["red", "green"]' to ["red", "green"]
	                if (_.first(value) === '[') {
	                    value = JSON.parse(value);
	                }

	                // parse the additional option and merge it into chart options
	                var option = key.split('.');
	                option.shift();
	                var length = option.length - 1;
	                var temp = this.chartOptions;
	                _.each(option, function(elem, idx) {
	                    temp[elem] = temp[elem] || {};
	                    if (idx == length) {
	                        temp[elem] = value;
	                    }
	            ...
	            this.chart = new Highcharts.chart(this.vizId, this.chartOptions);
	        },

	        /**
	         * Returns an object with default chart options for Highcharts.
	         *
	         * @param {Object} series: data series to be rendered by the chart.
	         */
	        getDefaultChartOptions: function(series) {
	            if (this.options.title.indexOf('%') > -1) {
	                var min = 0;
	                var max = 100;
	            }
	            ...
	            chartOptions.chart = {};
	            chartOptions.chart.type = this.options.chartType;
	            chartOptions.chart.height = 70;
	            chartOptions.chart.animation = false;
	            chartOptions.chart.backgroundColor = 'transparent';
	            chartOptions.chart.reflow = false;
	            chartOptions.chart.spacingBottom = 5;

	            chartOptions.colors = ['#7cb5ec'];
	            ...
	            chartOptions.yAxis.labels.style.fontSize = '10px';

	            chartOptions.xAxis = {};
	            chartOptions.xAxis.title = {};
	            chartOptions.xAxis.title.text = null;
	            chartOptions.xAxis.tickColor = 'transparent';
	            chartOptions.xAxis.labels = {};
	            chartOptions.xAxis.labels.enabled = false;

	            chartOptions.pane = {};
	            chartOptions.pane.startAngle = '-90';
	            chartOptions.pane.endAngle = '90';
	            chartOptions.pane.background = null;
	            chartOptions.pane.size = '90';
	            chartOptions.pane.center = ['50%', '85%'];
