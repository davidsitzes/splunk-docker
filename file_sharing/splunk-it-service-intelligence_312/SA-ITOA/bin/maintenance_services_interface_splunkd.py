...
logger = setup_logging("maintenance_services.log", "itsi.rest_handler_splunkd.maintenance_services_interface")
...
class MaintenanceServicesInterfaceSplunkd(PersistentServerConnectionApplication, SplunkdRestInterfaceBase):
    ...
    def _dispatch_to_provider(self, args):
        ...
        if not isinstance(args, dict):
            message = _('Invalid REST args received by maintenance services interface - {}').format(args)
        ...
        if not isinstance(rest_path, basestring):
            message = _('Invalid REST path received by maintenance services interface - {}').format(rest_path)
            raise ItoaValidationError(message=message, logger=logger)
