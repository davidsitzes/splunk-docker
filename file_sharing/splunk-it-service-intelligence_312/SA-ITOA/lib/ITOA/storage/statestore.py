...
logger = setup_logging("itsi.log", LOGGER)
...
class StateStore(object):
    ...
    def _set_batch_save_size_limit(self, session_key):
        """
    ...
    def check_payload_size(self, data_list, session_key, throw_on_violation=True):
        """
        ...
        self._set_batch_save_size_limit(session_key)
    ...
    def lazy_init(self, session_key):
        '''
        ...
        entries = []
        uri = "/servicesNS/" + self.owner + "/" + self.app + "/storage/collections/config/" + self.collectionname
        try:
    ...
    def create(self, session_key, owner, objecttype, data):
        """
        ...
        #Build the request
        uri = "/servicesNS/" + owner + "/" + self.app + "/storage/collections/data/" + self.collectionname
    ...
    def get(self, session_key, owner, objecttype, identifier, sort_key=None, sort_dir=None, filter_data={}, fields=None, skip=None, limit=None):
        """
    ...
    def edit(self, session_key, owner, objecttype, identifier, data):
        """
        ...
        curr_owner = owner
        uri = "/servicesNS/" + curr_owner + "/" + self.app + "/storage/collections/data/" + self.collectionname  + "/" + quote_plus(identifier)
        # These are the two fields that we require
        if '_key' not in data:
            data['_key'] = identifier
        data['object_type'] = objecttype

        response0 = self.get(session_key, curr_owner, objecttype, identifier)
        if response0 is None or len(response0) == 0: # object doesn't exist under input namespace
            if curr_owner != "nobody":
                message = _("Can't change permissions from 'app' to 'user' without cloning.")
                logger.error(LOG_PREFIX + message)
                raise StateStoreError(message)
            ...
            if curr_owner == "nobody":
                ...
                curr_owner = "nobody"
            response1 = self.get(session_key,curr_owner,objecttype,identifier)
            if response1 is None or len(response1) == 0: # object doesn't exist in either namespace, error
                ...
                self.delete(session_key, curr_owner, objecttype, identifier)
                # CREATE object in input name space
                return self.create(session_key, owner, objecttype, data)
    ...
    def delete(self, session_key, owner, objecttype, identifier):
        """
        ...
        uri = "/servicesNS/" + owner + "/" + self.app + "/storage/collections/data/" + self.collectionname  + "/" + quote_plus(identifier)
        try:
            response,content = rest.simpleRequest(
                    uri,
                    method="DELETE",
    ...
    def delete_all(self, session_key, owner, objecttype, filter_data):
        """
    ...
    def get_all(self, session_key, owner, objecttype, sort_key=None, sort_dir=None, filter_data = {}, fields=None, skip=None, limit=None):
        """
    ...
    def batch_save(self, session_key, owner, data):
        """
        ...
        @type data: list
        """
        self.check_payload_size(data, session_key)
        ...
        for range in self._save_ranges:
            parsed_content.extend(self._execute_save_request(session_key, owner, data[range[0]:range[1]]))
        self._save_ranges = []

        return parsed_content

    def _execute_save_request(self, session_key, owner, data):
        """
        ...
        """
        LOG_PREFIX = "[batch_save]"
        uri = "/servicesNS/" + owner + "/" + self.app + "/storage/collections/data/" + self.collectionname + "/batch_save"
        response,content = rest.simpleRequest(
                uri,
                method="POST",
                jsonargs=json.dumps(data),
