...
class Event(EventBase):
    ...
    def _get_from_index(self, event_ids, split_by="," , **kwargs):
        ...
        if not isinstance(event_ids, list):
            raise TypeError(_('Expecting event_ids to be a string or list'))
    ...
    def get_severity(self, events=None, event_ids=None, split_by=",", **kwargs):
        ...
        if not events and not event_ids:
            raise ValueError(_('Expecting either `events` or `event_ids`. Both'
                ' are None.'))
        if events and isinstance(events, dict):
            events = [events]
        if events and not isinstance(events, list):
            raise TypeError(_('Invalid type for `events`. Expecting list.'
    ...
    def update(self, blob, split_by=',', **kwargs):
        ...
        if not isinstance(blob, list):
            raise TypeError(_('Expecting `blob` to be a dict/list. Received:'
                    ' %s')%type(blob).__name__)

        if not blob:
            raise ValueError(_('Expecting `blob` to be non-empty.'))

        rval = []
        for group in blob:
            # validate/sanitize...
            if not isinstance(group, dict):
                raise TypeError(_('Expecting a dictionary. Received: '
                    '`%s`. Type: `%s`') % (group, type(group).__name__))
            if 'event_ids' not in group:
                raise KeyError(_('Expecting `event_ids` in your input.'))
    ...
    def update_severity(self, event_ids, severity, split_by=',', **kwargs):
        ...
        if isinstance(event_ids, basestring):
            if not event_ids.strip():
                raise ValueError(_('Expecting `event_ids` to contain some value.'
                        ' Received: %s') % event_ids)
            event_ids = event_ids.split(split_by)
        if not isinstance(event_ids, list):
            raise TypeError(_('Expecting `event_ids` to be of type basestring/'
                'list. Received: {}. Type: {}').format(event_ids,
                    type(event_ids).__name__))
        if not event_ids or not severity.strip():
            raise ValueError(_('Expecting non-empty list of `event_ids`. and valid'
                    ' severity string'))
    ...
    def create_tag(self, event_id, tag_value, raise_exceptions=False):
        ...
        if not isinstance(event_id, basestring):
            raise TypeError(_('Expecting `event_id` to be non-empty'
                ' basestring. Received: %s') % event_id)
        if not isinstance(tag_value, basestring):
            raise TypeError(_('Expecting `tag_value` to be non-empty'
    ...
    def update_tag(self, event_id, tag_id, tag_value):
        ...
        if not isinstance(tag_id, basestring):
            raise TypeError(_('Expecting `tag_id` to be non-empty'
    ...
    def create_comment(self, event_id, comment):
        ...
        if not isinstance(comment, basestring):
            raise TypeError(_('Expecting `comment` to be non-empty'
    ...
    def get_comment(self, comment_id):
        ...
        if not isinstance(comment_id, basestring):
            raise TypeError(_('Expecting `comment_id` to be non-empty'
    ...
    def update_ticket_info(self, event_ids, ticket_system, ticket_id,
        ...
        if isinstance(event_ids, basestring):
            event_ids = event_ids.split(',')
        if not isinstance(event_ids, list):
            raise TypeError(_('Expecting event_ids to be a list type. Received=%s.') % type(event_ids).__name__)
        if not event_ids:
            raise ValueError(_('Expecting event_ids to have at least 1 id. Received=%s.') % event_ids)
        rval = {
            'success':[],
            'failed':[]
            }
