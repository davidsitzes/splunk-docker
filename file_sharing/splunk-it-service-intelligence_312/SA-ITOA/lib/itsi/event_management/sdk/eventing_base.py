    ...
    def __init__(self, session_key, username=None, password=None, logger=None):
        if session_key:
            if not isinstance(session_key, basestring):
                raise TypeError(_('session_key must be string of non-zero length.'))
            if not session_key.strip():
                raise ValueError(_('session_key must be a non-zero length string.'))
        else:
            if any([
                ...
                raise ValueError(_('In the absense of session_key, expecting a valid '
                    ' non empty string, for both your username and password.'))
            session_key = getSessionKey(username, password)
            if not session_key:
                raise TypeError(_('Login failed. Seems like some credentials'
                        ' issue.'))
